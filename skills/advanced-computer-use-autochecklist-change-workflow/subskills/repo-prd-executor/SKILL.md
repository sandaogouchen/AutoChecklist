...
When this skill is invoked from [genfu-change-workflow](../../SKILL.md), inherit repository defaults and repo-specific hints from [repo profile](../../references/repo-profile.md) through the parent runtime config instead of inventing local defaults.
Inspect the repository, branches, diffs, and pull requests according to `runtime_mode`.
...
7. If this skill is running under the parent workflow, use the merged runtime config assembled from [repo profile](../../references/repo-profile.md) and [runtime config template](../../references/runtime-config-template.md).

## Workflow

1. Load the PRD and understand the current repo first. Prefer analysis-branch artifacts when available, then read relevant source files through the configured access mode.
...
5. Run the self-feedback loop before finalizing:
   ...
   - investigate large single-file code deletions in the real diff
...
- `runtime_mode=github_mcp`: use GitHub MCP for branch, PR, merge, and diff APIs.
- `runtime_mode=local_git`: use local git and the local repository.
