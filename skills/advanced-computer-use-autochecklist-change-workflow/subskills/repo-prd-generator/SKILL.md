...
Inspect the target repository and analysis branch according to `runtime_mode`.
...
6. If this skill is running under the parent workflow, use the merged runtime config assembled from [repo profile](../../references/repo-profile.md) and [runtime config template](../../references/runtime-config-template.md).
...
- `runtime_mode=github_mcp` means: inspect the repository and analysis artifacts through GitHub MCP.
- `runtime_mode=local_git` means: inspect the local repository and local analysis artifacts.
