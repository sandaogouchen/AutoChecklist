...
- **类型**: A-核心算法 + C-LLM集成
- **职责**: LLM 基于 checkpoint + evidence + 固定层级路径（Fixed hierarchy path）生成 TestCase
- **架构变更**: 从工厂闭包模式 (`build_draft_writer_node()`) 重构为类模式 (`DraftWriterNode`)，使用 `async def __call__` 实现节点接口
- **关键特性**:
- **`_SYSTEM_PROMPT`** 包含完整的 QA 测试用例撰写指南：
  - 6 个必填字段：title / steps / preconditions / expected_results / priority / category
  - 5 条编写原则：原子性、可执行性、可验证性、独立性、复用意识
  - **【路径与步骤衔接规范】（4 条规则）**:
    1. **衔接性**: steps 第一步应与路径最后一个节点的操作语义自然衔接，形成连贯操作链路
    2. **不重复**: 路径中已描述的操作（如"进入 XX 页面"、"选择 XX 目标"）不在 steps 中重复
    3. **聚焦细节**: steps 聚焦于路径未覆盖的具体交互细节（如"展开下拉框"、"输入数值"、"点击确认按钮"）
    4. **操作可执行性**: 每个 step 必须是具体的、可执行的操作指令，而非抽象描述
- **`_USER_PROMPT_TEMPLATE`**: 结构化模板，包含 Checkpoint ID/Title/Description 和 Fixed hierarchy path
- **`DraftWriterNode._format_path()`**: 静态方法，从 `path_mappings` 和 `outline_nodes` 构建 `" → "` 连接的可读路径字符串
- **`DraftWriterNode._generate_draft()`**: 异步方法，调用 LLM 生成单个 TestCase 草稿，包含 try/except 异常处理（返回 None 而非抛出异常）
- **Checklist 整合关键点**: 路径-步骤衔接规范确保了 outline 层级路径（包含动词化的 `display_text`）与生成的测试步骤之间形成连贯的操作链路，避免路径信息与步骤内容的重叠
### §3.11 evaluation.py
- **类型**: A-核心算法
- **职责**: 6 维度评估测试用例集质量
- **评估维度**:
...
- **类型**: B-流程编排
- **职责**: 调用 `attach_expected_results_to_outline()` 将完整 TestCase 数据挂载到 outline 树的 case 叶节点
- **角色**: 被动组装，不做智能整合
## §4 节点流水线
### 主图流水线
```
...
↓ ↓ ↓
CanonicalOutlineNode树 路径上下文注入(prompt) 挂载完整TestCase数据
↓ ↓ ↓
optimized_tree TestCase生成 最终ChecklistNode树
(含动词化display_text) (路径-步骤衔接) (_fill_node_from_testcase)
```
...
### §5.4 draft_writer 路径注入与步骤衔接机制分析
`DraftWriterNode._format_path()` 将 outline 层级路径构建为 `" → "` 连接的可读字符串注入 LLM prompt：
- **优势 1**: 为 LLM 提供结构化上下文，指导前置条件和用例组织
- **优势 2**: 路径中的动词化 `display_text`（如"进入 `Create ad group` 页面 → 配置 `Placements` 包含 `TikTok`"）与 `_SYSTEM_PROMPT` 中的路径-步骤衔接规范协同工作，确保生成的 steps 与路径操作形成连贯链路
- **风险 1**: 路径上下文过长 → prompt 膨胀 → 挤压生成空间
- **风险 2**: 固定路径 → 限制 LLM 对边界场景的创造性组织
- **风险 3**: 路径查找失败时（checkpoint 未匹配到 outline 节点）→ 降级为 `"(no path available)"`，质量断崖
- **风险 4**: 路径-步骤衔接规范的实际遵循度取决于 LLM 对指令的理解，无运行时校验
### §5.5 structure_assembler 的被动角色
- 执行 `attach_expected_results_to_outline()` → 基于 checkpoint_id 反向索引挂载完整 TestCase 数据
- 支持多 TestCase 映射同一 checkpoint（sibling 节点机制）
- 包含 graceful degradation（try/except 包裹）
- 不做任何智能整合（不合并相似 expected_results，不检测遗漏）
### §5.6 端到端质量损失链
```
checkpoint_outline_planner → evidence_mapper → draft_writer → structure_assembler
[高风险] [中风险] [中风险] [低风险]
· 层级不合理 · 证据错配 · 路径注入失效 · checkpoint_id 缺失
· 同义节点重复 · 遗漏映射 · 步骤与路径不衔接 · sibling 顺序不确定
· 单子链过深 · 生成质量波动
· display_text 未遵循
  动词规则
```
...
4. **display_text 动词规则校验**: 在 outline 生成后增加正则/规则校验，确认叶节点 display_text 以中文动词开头
#### 中期（2-4 周）
5. **混合方案**: 恢复 SemanticPathNormalizer 做预处理 + CheckpointOutlinePlanner 做精调
6. **PreconditionGrouper 语义升级**: 用 embedding 相似度替代关键词匹配
7. **双向验证**: outline 生成后反向验证每个 checkpoint 的归属正确性
8. **路径-步骤衔接质量评估**: 在 evaluation 节点增加 steps 与 path 的衔接度评分维度
#### 长期（1-2 月）
9. **多轮迭代 outline**: 参照 case generation 迭代循环，outline 也做多轮评估优化
10. **结构化模板库**: 构建常见测试场景的标准分类模板，作为 LLM 规划参考
11. **用户反馈闭环**: 记录人工对 outline 的修改历史，作为未来规划的微调信号
