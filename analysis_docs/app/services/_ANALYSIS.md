...
| 2 | `checkpoint_outline_planner.py` | ~15KB | `CheckpointOutlinePlanner` | LLM 驱动的 outline 层级规划 + TestCase 数据完整挂载 |
...
**职责**: 使用 LLM 对 checkpoint 列表进行层级 outline 规划，再将完整的 TestCase 数据挂载到 `case` 叶节点，产出完整的 `ChecklistNode` 树。

**关键方法**:

| 方法 | 签名 | 说明 |
|------|------|------|
| `plan_outline()` | `async plan_outline(checkpoints, research) → CanonicalOutlineNodeCollection` | 将 checkpoint 概要 + 研究上下文输入 LLM，获取结构化的 outline JSON |
| `map_checkpoints()` | `async map_checkpoints(outline, checkpoints) → CheckpointPathCollection` | 将每个 checkpoint 映射到 outline 树中的一条路径 |
| `build_optimized_tree()` | `build_optimized_tree(outline, path_collection, checkpoints) → list[ChecklistNode]` | 确定性地将路径映射合并为共享节点的 `ChecklistNode` 树 |
| `_parse_outline_node()` | 内部 helper | 递归解析 LLM 返回的原始 dict 为 `CanonicalOutlineNode` |
| `_outline_node_to_dict()` | 内部 helper | 将 `CanonicalOutlineNode` 序列化为 plain dict 用于 JSON 传输 |
| `_index_outline_nodes()` | 内部 helper | 递归索引所有 outline 节点的 `id → CanonicalOutlineNode` 查找表 |
| `_insert_path()` | 内部 helper | 将单个 checkpoint 按路径插入树中，复用已存在的同级节点 |
| `_stable_hash()` | 模块级 helper | 基于 SHA-256 的短哈希生成，用于去重标识 |
| `attach_expected_results_to_outline()` | 模块级函数 | 将完整 TestCase 数据挂载到 `case` 叶节点（含 sibling 扩展和 graceful degradation） |
| `_fill_node_from_testcase()` | 模块级 helper | 从单个 `TestCase` 填充 `ChecklistNode` 的 steps/preconditions/expected_results 等字段 |

**`_OUTLINE_SYSTEM_PROMPT` 详解**:

本次变更新增了 `display_text` 格式化规则，强制要求每个节点的 `display_text` 以中文操作动词开头，使用动宾结构：

| 规则 | 说明 |
|------|------|
| 动词开头 | 每个 `display_text` 必须以中文操作动词起始 |
| 动宾结构 | 使用动宾结构（Verb-Object）而非纯名词短语 |
| 按 kind 选择动词 | `page` → 进入/打开；`action` → 配置/选择/设置/输入/点击；`context` → 确认/确保/满足；`business_object` → 定位到/聚焦 |
| UI 元素保留英文 | UI 元素名、字段名、专有名词保留英文并用反引号包裹 |
| 父子连贯性 | 父节点到子节点应形成连贯的操作序列 |
...
**`attach_expected_results_to_outline()` 重构详解**:

此函数从旧版的"仅挂载 expected_results 文本"重构为"完整挂载 TestCase 数据"：

1. **反向索引构建**: `checkpoint_id → list[TestCase]`，使用 `defaultdict(list)` 实现
2. **递归遍历**: 内部函数 `_enrich_children()` 递归遍历整棵树
3. **case 节点填充**: 对 `node_type == "case"` 的节点，填充以下字段：
   - `steps` — 测试步骤
   - `preconditions` — 前置条件
   ...
   - `test_case_ref` — 源 TestCase ID
4. **多 TestCase 映射同一 checkpoint**: 第一个 TestCase 合并到原始节点，后续 TestCase 创建 sibling `case` 节点（ID 格式 `{node.id}__tc__{tc.id}`）
5. **Legacy 兼容**: 对 `group` 类型节点且有 `checkpoint_id` 的，仍创建 `expected_result` 叶子节点
6. **Graceful degradation**: 整个函数体包裹在 `try/except` 中，异常时记录 warning 并返回未修改的树

**`_fill_node_from_testcase()` 辅助函数**:

| 签名 | `_fill_node_from_testcase(node: ChecklistNode, tc: TestCase) → None` |
|------|------|
| 职责 | 将单个 TestCase 的字段批量赋值到 ChecklistNode |
| 字段映射 | `tc.steps → node.steps`, `tc.preconditions → node.preconditions`, `tc.expected_results → node.expected_results`, `tc.priority → node.priority`, `tc.category → node.category`, `tc.evidence_refs → node.evidence_refs`, `tc.id → node.test_case_ref` |
| 防御设计 | 使用 `getattr(tc, field, None) or ""` 避免属性缺失 |

**工作流程（更新后）**:

1. **Step 1 — LLM Outline 规划** (`plan_outline`): 将所有 checkpoint 的 ID/标题/描述 + 研究上下文组装为 prompt，要求 LLM 返回层级化的 outline JSON 数组
2. **Step 2 — Checkpoint 路径映射** (`map_checkpoints`): 将 outline 树和 checkpoint 列表输入 LLM，获取每个 checkpoint 到 outline 路径的映射
3. **Step 3 — 合并构建优化树** (`build_optimized_tree`): 确定性地将路径映射合并为共享节点的 `ChecklistNode` 树，相同路径段复用同一节点
4. **Step 4 — TestCase 数据挂载** (`attach_expected_results_to_outline`): 递归遍历树，将 TestCase 的完整数据（steps/preconditions/expected_results/priority/category/evidence_refs）填充到对应的 `case` 叶节点

**约束与限制**:
- LLM 单次调用处理所有 checkpoints，存在 context window 和规划质量的瓶颈
- outline JSON 解析依赖 LLM 严格遵循格式约定
- `display_text` 动词规则依赖 LLM 遵循 prompt 指令，无运行时校验
- 多 TestCase 映射同一 checkpoint 时的 sibling 节点排列顺序取决于 `test_cases` 列表顺序
...
│
▼
CheckpointOutlinePlanner.plan_outline()
│ LLM → CanonicalOutlineNode JSON
▼
CheckpointOutlinePlanner.map_checkpoints()
│ LLM → CheckpointPathCollection
▼
CheckpointOutlinePlanner.build_optimized_tree()
│ 确定性合并 → ChecklistNode 树
▼
attach_expected_results_to_outline()
│ checkpoint_id → TestCase 反向索引
│ _enrich_children() 递归遍历
│ _fill_node_from_testcase() 填充字段
│ 多 TestCase → sibling case 节点
▼
PreconditionGrouper
...
│
▼
CheckpointOutlinePlanner.plan_outline(checkpoints, research)
│ 将所有 checkpoint 概要 + 研究上下文组装为 prompt
...
│ │ node_type: "page" │
│ │ children: │
│ │ - display_text: "配置用户名密码" │
│ │ - display_text: "配置手机号验证码" │
│ └──────────────────────────────────┘
▼
CheckpointOutlinePlanner.map_checkpoints(outline, checkpoints)
│ LLM 将每个 checkpoint 映射到 outline 路径
▼
CheckpointOutlinePlanner.build_optimized_tree(outline, paths, checkpoints)
│ 确定性合并：相同路径段复用同一节点
▼
ChecklistNode 树 (含 case 叶节点)
│
▼
attach_expected_results_to_outline(tree, test_cases)
│ 1. 构建 checkpoint_id → list[TestCase] 反向索引
│ 2. _enrich_children() 递归遍历树
│ 3. case 节点 → _fill_node_from_testcase() 填充完整数据
│ 4. 多 TestCase → 创建 sibling case 节点
│ 5. try/except → graceful degradation
▼
PreconditionGrouper
...
- `display_text` 动词规则使得生成的 checklist 节点更具可操作性和可读性
- `attach_expected_results_to_outline` 的 graceful degradation 确保异常不阻断流水线
**现存问题** (详见 §5.3):
1. LLM 单次处理大量 checkpoint 时层级规划质量下降
2. PreconditionGrouper 使用关键词匹配而非语义理解，分组精度有限
3. `display_text` 动词规则依赖 LLM 遵循 prompt 指令，无运行时校验
...
│ │ │
│ │ ├── CheckpointOutlinePlanner.plan_outline(checkpoints, research)
│ │ │ ├── 组装 prompt: checkpoint 概要列表 + 研究上下文
│ │ │ ├── LLM 调用: → JSON 响应
│ │ │ ├── JSON 解析: → CanonicalOutlineNodeCollection
│ │ │ └── 返回 outline 骨架
│ │ │
│ │ ├── CheckpointOutlinePlanner.map_checkpoints(outline, checkpoints)
│ │ │ ├── 组装 prompt: outline JSON + checkpoint 列表
│ │ │ ├── LLM 调用: → JSON 响应
│ │ │ └── 返回 CheckpointPathCollection
│ │ │
│ │ └── CheckpointOutlinePlanner.build_optimized_tree(outline, paths, checkpoints)
│ │ ├── 索引所有 outline 节点
│ │ ├── 逐 checkpoint 按路径插入树
│ │ └── 返回 list[ChecklistNode]
│ │
│ ├── [Node] evidence_mapper
│ │ └── 将 evidence 映射到 ChecklistNode 节点（graph 层实现）
│ │
│ ├── [Node] draft_writer
│ │ ├── DraftWriterNode._format_path(): 构建层级路径字符串
│ │ │ └── 为每个 checkpoint 注入完整的层级路径上下文
│ │ │ 例: "进入 `Create ad group` 页面 → 配置 `Placements` 包含 `TikTok`"
│ │ └── LLM 调用: 基于路径上下文 + _SYSTEM_PROMPT（含路径-步骤衔接规范）生成 TestCase
│ │
│ └── [Node] structure_assembler
│ ├── 收集所有 draft_writer 产出的 TestCase
│ ├── attach_expected_results_to_outline() ← 完整 TestCase 数据挂载
│ │ ├── 构建 checkpoint_id → list[TestCase] 反向索引
│ │ ├── _enrich_children() 递归遍历
│ │ ├── _fill_node_from_testcase() 填充字段
│ │ └── 多 TestCase → sibling case 节点
...
| 变换节点 | 输入 | 输出 | 变换类型 |
|----------|------|------|----------|
| `plan_outline()` | `list[Checkpoint]` + `ResearchOutput` | `CanonicalOutlineNodeCollection` | LLM 生成（非确定性） |
| `map_checkpoints()` | `outline` + `list[Checkpoint]` | `CheckpointPathCollection` | LLM 生成（非确定性） |
| `build_optimized_tree()` | `outline` + `paths` + `checkpoints` | `list[ChecklistNode]` | 确定性合并 |
| `attach_expected_results` | `tree` + `list[TestCase]` | `list[ChecklistNode]` | 反向索引 + 递归填充（确定性） |
| `DraftWriterNode._generate_draft` | 路径上下文 + checkpoint | `TestCase` | LLM 生成（非确定性） |
| `PreconditionGrouper` | `ChecklistNode tree` | 增强的树（含分组节点） | 关键词匹配（确定性） |
#### §5.2.3 `attach_expected_results_to_outline` 匹配逻辑详解
此函数已从简单的字符串匹配重构为基于 `checkpoint_id` 的精确反向索引匹配：
```
1. 构建反向索引: checkpoint_id → list[TestCase]
使用 defaultdict(list)，遍历所有 test_cases
2. 递归遍历 _enrich_children(children):
对每个 node:
├── 若有子节点 → 先递归处理子节点
├── 若为 group 节点且有 checkpoint_id:
│ └── 为匹配的 TestCase 创建 expected_result 叶子节点（Legacy 兼容）
└── 若为 case 节点且有 checkpoint_id:
├── 查找 cp_to_cases[checkpoint_id]
├── 无匹配 → 保持原样
├── 第一个 TestCase → _fill_node_from_testcase(node, first_tc)
└── 后续 TestCase → 创建 sibling case 节点（ID: {node.id}__tc__{tc.id}）
3. 异常处理: try/except 包裹整个逻辑
└── 异常时 logger.warning() + 返回未修改的 tree
```
**相比旧版的改进**:
- 旧版: 基于可见路径 ID 列表在树中查找 group 节点 → 字符串匹配挂载 expected_results 文本
- 新版: 基于 checkpoint_id 精确匹配 → 填充完整 TestCase 数据（steps/preconditions/expected_results/priority/category/evidence_refs/test_case_ref）
- 新版支持多 TestCase 映射同一 checkpoint（sibling 节点机制）
- 新版增加 graceful degradation（try/except 包裹）
---
### §5.3 效果不佳的根本原因分析
#### §5.3.1 单次 LLM 规划的信息瓶颈
**问题描述**: `CheckpointOutlinePlanner.plan_outline()` 将所有 checkpoint 的概要一次性输入 LLM，要求其返回完整的层级 outline。
**影响**:
...
**问题描述**: `attach_expected_results_to_outline()` 在重构后已从简单字符串匹配升级为基于 `checkpoint_id` 的精确反向索引，但仍存在部分边界问题。
**改进后的状态**:
| 维度 | 旧版 | 新版 |
|------|------|------|
| 匹配机制 | 可见路径 ID + 字符串包含 | `checkpoint_id` 精确匹配 |
| 挂载内容 | 仅 `expected_results` 文本 | 完整 TestCase 数据（7 个字段） |
| 多 TestCase | 不支持 | sibling case 节点 |
| 异常处理 | 无 | try/except graceful degradation |

**残余问题**:
- 若 `checkpoint_id` 未正确设置（LLM 生成 TestCase 时遗漏），仍会导致挂载失败
- sibling 节点的排列顺序依赖 `test_cases` 列表输入顺序，非确定性
- Legacy 兼容路径（group 节点创建 expected_result 叶子）与新逻辑并存，增加维护复杂度
---
#### §5.3.5 缺乏反馈循环
...
| **LLM 使用** | 两次（路径提取 + 归一化） | 两次（outline 规划 + checkpoint 路径映射） |
| **确定性** | 合并步骤确定性，归一化非确定性 | 规划非确定性，树构建和挂载确定性 |
| **可调试性** | 中（可检查中间路径） | 低（outline 为黑盒输出） |
| **扩展性** | 路径数量增长时 trie 性能稳定 | checkpoint 数量增长时 LLM 质量下降 |
| **层级合理性** | 差（取决于路径归一化质量） | 中（取决于 LLM 理解深度） |
| **覆盖完整性** | 高（每个 checkpoint 必有路径） | 高（每个 checkpoint 必有映射路径） |
