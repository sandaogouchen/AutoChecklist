...
def build_draft_writer_node(llm_client: LLMClient):
    ...
    def draft_writer_node(state: CaseGenState) -> CaseGenState:
        ...
        # ---- 模板驱动生成支持：将模板规则注入 draft 生成 prompt ----
        ...
        if template_data:
            try:
                ...
                template_section = template_obj.format_for_draft_prompt()
                if template_section:
                    prompt_lines.insert(0, template_section)
                    logger.info("已将模板规则注入 draft 生成 prompt")
            except Exception:
