...
| 字段 | 类型 | 说明 |
|------|------|------|
| `knowledge_context` | `str` | 格式化后的知识检索文本，由 `retriever.format_retrieval_result()` 生成，注入 context_research 节点的 prompt |
| `knowledge_sources` | `list[str]` | 检索命中的知识文档 ID 列表 |
| `knowledge_retrieval_success` | `bool` | 知识检索是否成功执行 |

### 设计说明

1. **三字段对应**: 三个字段与 `retriever.retrieve_knowledge()` 返回的三元组 `(context, sources, success)` 一一对应
2. **字段分组**: 源码中以 `# ---- 知识检索字段 ----` 注释与其他字段分组，与现有 `mandatory_skeleton` 等字段风格一致
3. **降级友好**: `knowledge_context` 为空字符串时，下游 `context_research` 节点不注入额外知识上下文，对现有流程零影响
4. **无默认值要求**: TypedDict 字段不设默认值，由 knowledge_retrieval 节点负责写入
