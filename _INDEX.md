...
4. **expected_results 挂载已升级**: 基于 checkpoint_id 精确反向索引 + 完整 TestCase 数据填充 + sibling 节点 + graceful degradation
...
| structure_assembler 节点级集成测试 | 核心挂载函数已有测试（`test_attach_expected_results.py`），节点级行为未覆盖 | P1 |
