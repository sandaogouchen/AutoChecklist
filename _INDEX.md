...
> **最新同步**: PR #36 `feat/mr-code-analysis-integration` — MR 代码分析集成, 6 个分析文件更新（domain、nodes、services、graphs、config、本索引）
...
│  │ checkpoint_evaluator → [mr_analyzer] →                   │ │
│  │ [mr_checkpoint_injector] → checkpoint_outline_planner →  │ │
│  │ evidence_mapper → draft_writer → structure_assembler →   │ │
│  │ [coco_consistency_validator]                              │ │
│  └──────────────────────────────────────────────────────┘ │
│                                                │              │
│                                                ▼              │
│                          reflection → 最终输出                │
...
| [app/domain/_ANALYSIS.md](./app/domain/_ANALYSIS.md) | 领域模型层（Pydantic v2 + MR 模型） | 11 | 26.0 KB | ★★★ |
...
| [app/nodes/_ANALYSIS.md](./app/nodes/_ANALYSIS.md) | 工作流节点（15 个节点，含 3 个 MR 分析节点） | 15 | 12.2 KB | ★★★ |
...
| [app/services/_ANALYSIS.md](./app/services/_ANALYSIS.md) | 业务服务层（含 Checklist 核心 + 强制骨架构建 + MR 分析工具） | 17 | 46.0 KB | ★★★ |
...
PR #36 引入了 MR（Merge Request）代码分析能力，允许在 PRD 驱动的测试用例生成流程中注入代码变更上下文。系统从 MR diff 中提取结构化代码事实，通过 agentic search 补充上下文，转换为 Checkpoint 注入现有流水线，并通过 Coco API 执行代码一致性验证。
...
│            │                        │
│            ▼                        │
│ mr_checkpoint_injector              │
│ ├── 事实分组 + LLM 转换             │
│ └── 与 PRD checkpoints 去重合并     │
└─────────────────────────────────────┘
│
▼
现有 checkpoint 流水线 (outline → evidence → draft → structure)
│
▼
┌─────────── 一致性验证 ──────────────┐
│ coco_consistency_validator           │
...
│
▼
CodeConsistencyCheck → TestCase / Checkpoint
```
...
| 文件 | 层级 | 行数 | 说明 |
|------|------|------|------|
| `app/domain/mr_models.py` | 领域模型 | ~295 | 11 个 Pydantic 模型（MR 元数据、代码事实、一致性校验） |
| `app/nodes/mr_analyzer.py` | 工作流节点 | ~763 | 核心 MR 分析节点（diff 解析 + LLM 事实提取 + agentic search） |
| `app/nodes/mr_checkpoint_injector.py` | 工作流节点 | ~308 | 代码事实→Checkpoint 转换与合并注入 |
| `app/nodes/coco_consistency_validator.py` | 工作流节点 | ~393 | Coco Task 2 代码一致性验证 |
| `app/services/codebase_tools.py` | 服务层 | ~627 | 5 个 agentic search 工具集 |
| `app/services/coco_client.py` | 服务层 | ~482 | Coco API 异步客户端（并发控制 + 重试） |
| `app/services/coco_response_validator.py` | 服务层 | ~335 | Coco 响应三层验证 |

### 变更影响矩阵

| 变更文件/模块 | 影响摘要 |
|--------------|----------|
| `app/domain/mr_models.py` (新) | 11 个 MR 分析 Pydantic 模型 |
| `app/domain/state.py` | 3 个 GlobalState MR 分析字段 |
| `app/domain/api_models.py` | MRRequestConfig + frontend_mr/backend_mr 字段 |
| `app/domain/case_models.py` | TestCase +tags +code_consistency 字段 |
| `app/domain/checkpoint_models.py` | Checkpoint +code_consistency 字段 |
| `app/nodes/mr_analyzer.py` (新) | 763 行，MR 分析核心节点 + agentic search |
| `app/nodes/mr_checkpoint_injector.py` (新) | 308 行，代码事实→Checkpoint 转换 |
| `app/nodes/coco_consistency_validator.py` (新) | 393 行，Coco Task 2 验证 |
| `app/nodes/structure_assembler.py` | TODO: MR source annotation 预留 |
| `app/nodes/scenario_planner.py` | MR 上下文注入场景规划 |
| `app/nodes/checkpoint_generator.py` | MR 代码事实注入 prompt |
| `app/nodes/draft_writer.py` | XMind 适配 + 代码变更上下文 |
| `app/services/codebase_tools.py` (新) | 627 行，5 个搜索工具 |
| `app/services/coco_client.py` (新) | 482 行，Coco API 客户端 |
| `app/services/coco_response_validator.py` (新) | 335 行，三层验证 |
| `app/graphs/case_generation.py` | 子图拓扑 +3 条件节点 |
| `app/graphs/main_workflow.py` | 桥接 +3 字段映射，build_workflow +3 参数 |
| `app/config/settings.py` | CocoSettings 嵌套类 + 4 个 MR 配置字段 |

### 降级策略

| 场景 | 行为 |
|------|------|
| `enable_mr_analysis=False` | MR 节点不注入工作流，零开销 |
| MR URL 无效或 diff 获取失败 | mr_analyzer 返回空结果，后续节点安全跳过 |
| Agentic search 失败 | 保留已提取的代码事实，不阻断 |
| `enable_coco_validation=False` | Coco 节点不注入，跳过一致性验证 |
| Coco API 不可用 | health_check 失败，返回空结果 |
| Coco 响应验证失败 | 标记为 low_confidence，不阻断流水线 |

### 详细分析索引

- [app/domain/_ANALYSIS.md §7](./app/domain/_ANALYSIS.md) — MR 领域模型（11 个模型 + 依赖关系更新）
- [app/nodes/_ANALYSIS.md §7](./app/nodes/_ANALYSIS.md) — MR 分析节点（3 新 + 4 修改 + 流水线更新）
- [app/services/_ANALYSIS.md §7](./app/services/_ANALYSIS.md) — MR 服务层（搜索工具 + Coco 客户端 + 验证器）
- [app/graphs/_ANALYSIS.md §6](./app/graphs/_ANALYSIS.md) — 工作流拓扑变更（+3 条件节点 + 桥接扩展）
- [app/config/_ANALYSIS.md §7](./app/config/_ANALYSIS.md) — CocoSettings + MR 配置字段
## 核心数据流
...
| 12 | `build_workflow()` 参数膨胀（6+ 个可选节点参数） | graphs/ | 中 | [graphs §6](./app/graphs/_ANALYSIS.md) |
| 13 | `mr_code_facts` 双向桥接映射增加维护复杂度 | graphs/ | 中 | [graphs §6](./app/graphs/_ANALYSIS.md) |
| 14 | MR source annotation 待实现（structure_assembler TODO） | nodes/ | 低 | [nodes §7.4](./app/nodes/_ANALYSIS.md) |
